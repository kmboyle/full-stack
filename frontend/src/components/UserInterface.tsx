...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    ...
    const buttonColors: { [key: string]: string } = {
        rust: 'bg-oragne-700 hover:bg-orange-600',
    }
    ...
    return (
      <div className = {`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
        ...
        <h2 className="text-x1 font-bold text-center text-whit mb-6">{`${backendName.charAt(0).toUpperCase() + backendName.substring(1) + ' Backend'}`}</h2>
        ...
        <div className="space-y-4">
            {users.map((user) => (
                <div key={user.id} className="flex items-center justify-between bg-white p-4 rounded shadow">
                    ...
                    <button onClick={() => deleteUser(user.id)} className={`${btnColor} text-white py-2 px-4 bg-red-500 rounded hover:bg-red-600`}> 
                        Delete User
                    </button>
                </div>
            ))}
